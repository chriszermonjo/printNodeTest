...
class PDFGenerator {
  ...
  async generateWorkOrderPDF(workOrderData) {
    ...
    // Check if workOrderData is an array
    if (Array.isArray(workOrderData)) {
      // Handle multiple work orders
      const htmlContent = await this.generateMultipleWorkOrdersHTML(
        workOrderData
      );
      await page.setContent(htmlContent, { waitUntil: "networkidle0" });

      // Generate PDF buffer for multiple work orders
      ...
      // Handle single work order (existing logic)
  ...
  // Generate HTML content for multiple work orders
  async generateMultipleWorkOrdersHTML(workOrdersArray) {
    let allWorkOrdersHTML = "";

    for (let i = 0; i < workOrdersArray.length; i++) {
      const workOrderData = workOrdersArray[i];

      // Generate QR code and barcode for each work order
      const qrCodeDataURL = await this.generateQRCode(
        workOrderData.orderNumber
      );
      const barcodeDataURL = await this.generateBarcode(
      ...
      // Generate HTML for this work order
      const workOrderHTML = this.generateWorkOrderHTML(
        workOrderData,
        qrCodeDataURL,
        barcodeDataURL,
        designIdQRCode
      ...
      const bodyMatch = workOrderHTML.match(/<body[^>]*>([\s\S]*?)<\/body>/);
      if (bodyMatch) {
        allWorkOrdersHTML += bodyMatch[1];

        // Add page break between work orders (except for the last one)
        if (i < workOrdersArray.length - 1) {
          allWorkOrdersHTML += '<div style="page-break-after: always;"></div>';
        }
      }
    }

    // Wrap all work orders in a complete HTML document
    return `
      <!DOCTYPE html>
      <html>
        <head>
          <meta charset="UTF-8">
          <title>Multiple Work Orders</title>
          <style>
            body {
              font-family: Arial, sans-serif;
              margin: 0;
